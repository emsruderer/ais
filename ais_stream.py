...
chain = FilterChain([
    ...
    MessageTypeFilter(1, 2, 3, 18, 19, 24),
    ])

def pretty_print(tracks):
    """Pretty print a list of tracks to stdout."""
    headers = ['mmsi', 'callsign', 'shipname', 'lat', 'lon', 'cpa', 'tcpa', 'dist', 'bear']
    ...
    with CPATracker() as tracker:
        ...
        with TCPConnection('localhost', 10110) as ais_stream:
            for msg in ais_stream:
                decoded = msg.decode()
                cpa_res = { "cpa": None, "tcpa": None, "distance": None, "bearing": None }
                if decoded.msg_type in [1,2,3,18]:

                    cpa_res = cpa_tcpa( known_ship.get_mmsi(), known_ship.get_latitude(), known_ship.get_longitude(),
                ...
                elif decoded.msg_type in [19,24]:
                    tracker.update(decoded, cpa_res)
                if VERBOSE:
                    latest_tracks = tracker.n_latest_tracks(5)
...
if __name__ == '__main__':
    #VERBOSE = True
    print(VERBOSE)
    my_ship = Ship(244030153, 53.26379, 7.39738
, 0, 5.0)
    my_ship.start()
